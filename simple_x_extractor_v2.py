...
def parse_snapshot(snapshot, search_query):
    ...
    for article_text in articles:
        ...
        # Text is everything after the date and before engagement metrics
        text_match = re.search(r'\d{4}|\d+[hm]\s+(.+?)\s+\d+\s+replies', article_text, re.DOTALL)
        if text_match:
            post['text'] = text_match.group(1).strip()
        else:
            # Fallback: split by engagement keywords
            parts = re.split(r'\d+\s+(?:replies|likes|reposts)', article_text)
            if len(parts) > 0:
                # Take the longest part that's not just metadata
                for part in parts:
                    if len(part) > 30 and '@' not in part[:20]:  # Not starting with handle
                        post['text'] = part.strip()[:500]
                        break
        
        # If still no text, use article text as-is
        if 'text' not in post:
            post['text'] = article_text[:500]
...
def main():
    ...
    with open('/tmp/snapshot1.txt', 'w') as f:
        f.write(snapshot1)
    ...
    # Save snapshot for debugging
    with open('/tmp/snapshot2.txt', 'w') as f:
        f.write(snapshot2)
    ...
    for post in all_posts:
        ...
        replies = int(post.get('replies', '0')) if post.get('replies', '0').isdigit() else 0
        reposts = int(post.get('reposts', '0')) if post.get('reposts', '0').isdigit() else 0
