...
def parse_posts_from_snapshot(snapshot_text, search_query):
    posts = []
    # Split by article tags - look for the article element content
    ...
    for article_text in articles:
        ...
        # Find where the text starts (after handle)
        text_start = 0
        if 'handle' in post:
...
if __name__ == "__main__":
    ...
    for post in high_engagement:
        print(f"  - {post.get('author')} ({post.get('handle')}): {post.get('likes')} likes")
    
    # Debug: Print first few posts
    if posts:
        print(f"\nFirst post sample:")
        print(f"  Author: {posts[0].get('author')}")
        print(f"  Handle: {posts[0].get('handle')}")
        print(f"  Date: {posts[0].get('date')}")
        print(f"  Text: {posts[0].get('text', '')[:100]}...")
        print(f"  URL: {posts[0].get('url')}")
        print(f"  Likes: {posts[0].get('likes', 'N/A')}")