...
export function getFoodsByCategory(category: FoodCategory): FoodItem[] {
  return KERALA_FOODS.filter(f => f.category === category);
}

export function getSoftFoods(): FoodItem[] {
  return KERALA_FOODS.filter(f => f.texture === "soft" || f.texture === "pureed");
}

export function getLowSodiumFoods(): FoodItem[] {
  return KERALA_FOODS.filter(f => f.isLowSodium);
}
