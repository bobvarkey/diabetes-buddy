...
import algorithmStepsImg from "@/assets/htn-algorithm-steps.jpg";
import comorbidityMatrixImg from "@/assets/htn-comorbidity-matrix.jpg";
...
export default function HtnAlgorithmFlowchart() {
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({ stepIdx, branchId, checked: Array.from(checked) })
      );
    } catch {
  ...
  const recommendedClasses =
    stepIdx === 0 && branchId !== "default" ? branch.preferred : currentStep.classes;
    ...
    <Card className="border-primary/20">
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Pick a comorbidity branch, then walk the escalation ladder. Tick the classes you have
          prescribed — the panel auto-suggests the next escalation step.
        </p>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <figure className="rounded-lg border bg-muted/20 overflow-hidden">
            <div className="relative">
              ...
              />
              {/* Clickable step hotspots (percent-positioned over the 4 boxes) */}
              {[
                ...
                <button
                  ...
                  className={`absolute left-[3%] right-[3%] rounded-lg transition-all cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-primary ${
                    stepIdx === h.idx
                      ? "ring-4 ring-primary/70 bg-primary/10"
        ...
        {/* One-click safe-prescribing shortcut */}
          ...
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div className="flex-1 min-w-[200px]">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                One click swaps the algorithm to lactation- and pregnancy-compatible options
                (labetalol, nifedipine, enalapril, methyldopa).
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button
                size="sm"
                variant={branchId === "pregnancy" ? "default" : "outline"}
                onClick={() => {
                  setBranchId("pregnancy");
                  setStepIdx(0);
                  setChecked(new Set());
                }}
              >
              ...
              <Button
                size="sm"
                variant={branchId === "lactation" ? "default" : "outline"}
                onClick={() => {
                  setBranchId("lactation");
                  setStepIdx(0);
                  setChecked(new Set());
                }}
              >
                ...
                <div key={s.id}>
                  <button
                    ...
                    className={`w-full text-left rounded-lg border-2 p-3 transition-all ${
                      active ? toneClasses[s.tone] + " ring-2 ring-primary/40" : toneClasses[s.tone]
                    } ${past ? "opacity-60" : ""} hover:ring-2 hover:ring-primary/30`}
                  >
                    <div className="flex items-center justify-between mb-1 gap-2 flex-wrap">
                      <Badge variant="outline" className={`${toneBadge[s.tone]} font-mono`}>
                        {s.step}
                      </Badge>
        ...
        {/* Live checklist of recommended classes for current node */}
        <div className="rounded-lg border-2 border-primary/30 bg-primary/5 p-4">
          <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
            <div className="flex items-center space-x-2">
              <CheckCircle2 className="h-4 w-4 text-primary" />
              <h4 className="text-sm font-semibold text-foreground">
                3. Recommended drug classes — {currentStep.step} · {branch.comorbidity}
              </h4>
            </div>
            <Badge variant="outline" className="text-xs">
              {checked.size} / {recommendedClasses.length} prescribed
            </Badge>
          </div>
          ...
          {currentStep.note && (
            <p className="text-xs text-muted-foreground mt-3 italic">{currentStep.note}</p>
          )}
        </div>

        {/* Auto-suggested next escalation */}
        <div className="rounded-lg border bg-muted/20 p-4">
          ...
          {nextStep ? (
            <p className="text-xs text-muted-foreground">
              If BP remains uncontrolled after 1–2 months at <strong>{currentStep.step}</strong>,
              advance to <strong>{nextStep.step} — {nextStep.label}</strong>:
              <span className="block mt-1 text-foreground">
                {nextStep.classes.join("  +  ")}
              </span>
            </p>
          ) : (
            <p className="text-xs text-muted-foreground">
              Refer to a hypertension specialist. Re-evaluate adherence, secondary causes, and
              consider device-based therapy.
            </p>
          )}
        </div>
