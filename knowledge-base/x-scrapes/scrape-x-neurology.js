...
const OUTPUT_PATH = '/Users/bobvarkey/.openclaw/workspace/knowledge-base/x-scrapes/x-neurology-2026-05-22.md';
...
async function extractPosts(page) {
  return await page.evaluate(() => {
    ...
    articles.forEach((article) => {
      ...
      // Find author link: contains a span starting with @
      ...
      const parseBtn = (btn) => {
        ...
        const match = txt.match(/([0-9.]+)\s*([KM]?)/i);
        if (match) {
...
async function main() {
  ...
  while (allPosts.length < 10 && stallCount < maxStalls) {
    ...
    // Scroll the main document to load more
    await page.evaluate(() => {
