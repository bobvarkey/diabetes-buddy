...
(async () => {
  ...
  // Robust load: retry goto + wait for article, detect login wall
  let loaded = false;
  for (let attempt = 0; attempt < 3 && !loaded; attempt++) {
    await page.goto(SEARCH_URL, { waitUntil: 'domcontentloaded', timeout: 60000 }).catch(e => console.log('goto warn: ' + e.message));
    await sleep(3000);
    // Click "See new posts" if present, then scroll to trigger lazy render
    try {
    ...
    })).catch(e => ({ url: 'ERR', title: e.message, hasArticle: false, isLogin: false }));
